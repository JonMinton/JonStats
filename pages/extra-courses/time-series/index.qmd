...
The process of **integrating the differences**, including a series of forecast values from the time series models, constitutes this inverse transform function in the context of time series modelling, as we saw in the [previous section on integration](#integration).
...
The last three sections have covered three of the main techniques - [autoregression](#autoregression), [integration](#integration), and [moving average](#the-moving-average-model-in-context-of-arima) modelling - which combine to form the ARIMA model framework for time series analysis. 
...
In this case the intercepts for both series are positive and statistically significant: over the long term, GDP has grown on average around 2% each year, and life expectancy by around 0.28%. As [the blog post this relates to](https://blog.jonminton.net/posts/still-the-economy/index.html) makes clear, however, these long-term trends may no longer apply.
