...
For an interactive treatment of where the numbers in those summary tables come from — Wald tests as estimate-over-standard-error, likelihood-ratio tests as comparing two peaks, and model comparison with AIC/BIC — see the GLM Dashboard's [Statistical Inference section](https://stats-board.jonminton.net/inference/); its [permutation-test page](https://stats-board.jonminton.net/hacker-stats/permutation.html) builds a null distribution by counting, exactly as described below.

Whether using resampling and bootstrapping methods, or more conventional methods, the underlying aim and approach in hypothesis testing is the same. There are always the same elements in play, even if the approaches used to derive, estimate or represent some of these elements differ between methods. These elements comprise: 
...
To help think further about the latter point, let's return to [the model specification that was the focal example of the third section of my statistics series](../../main-course/complete-simulation-example/index.qmd). This specification is as follows:
...
Many of the individual elements discussed in this post have been introduced or utilised in earlier posts I've written. For example I first discussed the overreliance on interpreting individual coefficients as quantities of interest in [this post](../../main-course/intro-to-glms/index.qmd) on beta values; I made use of terminology like restricted and unrestricted model, and AIC and anova, in [this post](../../main-course/complete-simulation-example/index.qmd) on selecting a model specification; and I introduced hypothesis testing on [this post](../hacker-stats/infer-introduction/index.qmd) on the infer package. However this is the first post where I've tried to focus on the concepts of P values and statistical significance of the type most people are familiar with. I hope this post therefore manages to tie these disparate pieces together a bit more coherently.[^claude-pvalues]

[^claude-pvalues]: **Note from Claude:** The issues with p-value misuse and "stargazing" discussed here are central to both classical statistics and modern machine learning. The American Statistical Association's 2016 statement on p-values and subsequent calls to "retire statistical significance" reflect growing recognition of these problems. In ML, model selection methods have evolved beyond p-values: cross-validation (splitting data into training/validation/test sets) provides robust performance estimates without hypothesis testing; information criteria like AIC/BIC mentioned here connect to regularization penalties in ridge/lasso regression; and the bias-variance tradeoff framework (underfitting vs overfitting) provides clearer intuition than "degrees of freedom budgeting." Python's scikit-learn ecosystem emphasizes these ML approaches through sklearn.model_selection (GridSearchCV, cross_val_score) and sklearn.feature_selection modules. @mcelreath2020statistical and @efron2016computer both critique overreliance on p-values while connecting classical methods to modern computational approaches.
