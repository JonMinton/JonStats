...
[Section One](../intro-to-glms/index.qmd) of this course introduced generalised linear models (GLMs) and statistical simulation. [Section Two](../likelihood-and-simulation-theory/index.qmd) then delved more into the underlying theory and technicalities involved in implementing GLMs and using them for simulation. 
...
Both report the same number of degrees of freedom ('df'), which shouldn't be surprising as they involve the same number of parameters. But the log likelihood for `mod_02` is higher, which like the Adjusted R-squared metric suggests a better fit. 
...
But we don't *need* to use `optim()` directly in order to recover these quantities. Instead we can get them from the standard model objects produced by either `lm()` or `glm()`. For step-by-step derivations of log-likelihoods for different GLM families, see the GLM Dashboard advanced tutorials: [Gaussian log-likelihood](https://stats-board.jonminton.net/tutorials/01-gaussian/advanced.html) and [Poisson log-likelihood](https://stats-board.jonminton.net/tutorials/03-poisson/advanced.html). Let's check this out...
...
[^4]: These are the values produced the first time I ran the simulation. They are likely to be a little different each time, so may not be identical to the number of decimal places reported when I next render this document. These estimates are *approximations*. 
...
[^5]: Because the simulation approach relies on random numbers, the draws will never be the same unless the same random number seed is using using `set.seed()`. However with more simulations, using the `n` parameter from `mvrnorm`, the distributions of estimates should become ever closer to each other. 
...
In the analogies above, *the marble is to frequentist statistics as the jumping bean is to Bayesian statistics*. A technical distinction between the marble and the jumping bean is that *the marble converges towards a point* (meaning it reaches a point of rest on the surface) whereas *the jumping bean converges towards a distribution* (meaning it never rests). The [GLM Dashboard terrain visualisations](https://stats-board.jonminton.net/optimization/) bring this analogy to life — you can watch algorithms navigate a real terrain surface, seeing how gradient-based methods (the marble) converge to a single peak, while [MCMC methods (the jumping bean) explore the surface continuously](https://stats-board.jonminton.net/optimization/mcmc.html).[^claude-optimization] And to see the *uncertainty* payoff of the two framings side by side — Hessian ellipse against Metropolis walker, with Wald and credible intervals compared numerically on the same small-sample model — see [Beyond the Quadratic: Bayesian Uncertainty](https://stats-board.jonminton.net/inference/bayes.html).

[^claude-optimization]: **Note from Claude:** The marble and jumping bean metaphors map directly onto optimization algorithm families used throughout machine learning. Gradient descent variants (the "marble") include SGD, Adam, and RMSprop—all converging to point estimates by following the fitness surface downhill. These power neural network training in frameworks like PyTorch (`torch.optim.SGD`, `torch.optim.Adam`) and TensorFlow (`tf.keras.optimizers`). The "jumping bean" family encompasses not just MCMC samplers but also genetic algorithms, simulated annealing, and particle swarm optimization—methods that explore distributions rather than converging to points. For Bayesian modeling in Python, PyMC (`pymc.sample()` using NUTS, a variant of HMC) and PyStan mirror the R packages discussed here. The key insight: whether fitting GLMs, training neural networks, or optimizing hyperparameters, you're always searching a fitness landscape—the choice of algorithm determines whether you get a point estimate (marble) or a distribution (jumping bean).
...
Now how would we build a Bayesian equivalent of this? Firstly let's load (and if necessary install[^8])  `rstanarm`.

[^8]: `rstanarm` has a lot of dependencies. It's the friendly, cuddly face of a *beast*!
```{r}
library(rstanarm)
