...
In the analogies above, *the marble is to frequentist statistics as the jumping bean is to Bayesian statistics*. A technical distinction between the marble and the jumping bean is that *the marble converges towards a point* (meaning it reaches a point of rest on the surface) whereas *the jumping bean converges towards a distribution* (meaning it never rests). The [GLM Dashboard terrain visualisations](https://jonminton.github.io/glm-dashboard-explainer/optimization/) bring this analogy to life — you can watch algorithms navigate a real terrain surface, seeing how gradient-based methods (the marble) converge to a single peak, while [MCMC methods (the jumping bean) explore the surface continuously](https://jonminton.github.io/glm-dashboard-explainer/optimization/mcmc.html).[^claude-optimization]
