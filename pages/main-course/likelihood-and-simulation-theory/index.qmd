...
But we don't have to do this. We can instead use an algorithm to take candidate parameters, try them out, then make new candidate parameters and try them out, for us. Much as a taxi driver needs to know where to meet a passenger, but doesn't want the passenger to tell them exactly which route to take, we just need to specify a starting set of values for the parameters to optimise. The [GLM Dashboard optimisation visualisations](https://stats-board.jonminton.net/optimization/) show this process animated on a real terrain — you can watch algorithms climb from starting points to peaks, seeing exactly how different optimisation strategies navigate the surface. R's standard way of doing this is with the `optim` function. Here's it in action:
...
Probe the curvature of a likelihood surface yourself, and watch the Hessian become standard errors and a confidence ellipse, in the GLM Dashboard's interactive companion to this section: [Standard Errors from Curvature](https://stats-board.jonminton.net/inference/curvature.html).
...
An interactive version of this exact exercise — three probes from the summit, filling in the Hessian matrix cell by cell, with the 45 degree probe revealing the covariance term — is on the [GLM Dashboard](https://stats-board.jonminton.net/inference/curvature.html).
...
[Earlier in this section](#likelihood-for-linear-regression), we used this same dataset to show how the log likelihood varies for various, equally spaced, candidate values for $\beta_0$ and $\beta_1$ (having fixed $\eta = \exp({\sigma^2})$ at its true value). This led to the followng map of the landscape[^8]
