...
::: {.callout-tip}
## See it in action
Watch gradient ascent and Newton-Raphson algorithms navigate a likelihood surface in the [GLM Dashboard optimisation visualisations](https://jonminton.github.io/glm-dashboard-explainer/optimization/).
:::
...
But we don't have to do this. We can instead use an algorithm to take candidate parameters, try them out, then make new candidate parameters and try them out, for us. Much as a taxi driver needs to know where to meet a passenger, but doesn't want the passenger to tell them exactly which route to take, we just need to specify a starting set of values for the parameters to optimise. The [GLM Dashboard optimisation visualisations](https://jonminton.github.io/glm-dashboard-explainer/optimization/) show this process animated on a real terrain — you can watch algorithms climb from starting points to peaks, seeing exactly how different optimisation strategies navigate the surface. R's standard way of doing this is with the `optim` function. Here's it in action:
