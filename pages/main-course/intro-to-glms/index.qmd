...
Pretty much all statistical model fitting involves iterating along this $g \to \beta$ and $\beta \to g$ feedback loop until some kind of condition is met involving minimising $\delta$.[^claude-ml]
...
Practice choosing the right distribution and link function for different data types with the [GLM Dashboard interactive tutorials](https://jonminton.github.io/glm-dashboard-explainer/).
:::
...
Each family has a default `link` argument, and for this `gaussian` family, this link is the identity function. To see how each of these families works in practice — choosing predictors, link functions, and distributions step-by-step — try the corresponding [GLM Dashboard](https://jonminton.github.io/glm-dashboard-explainer/) tutorials: [Gaussian](https://jonminton.github.io/glm-dashboard-explainer/tutorials/01-gaussian/systematic.html), [Binomial/Logistic](https://jonminton.github.io/glm-dashboard-explainer/tutorials/02-logistic/systematic.html), and [Poisson](https://jonminton.github.io/glm-dashboard-explainer/tutorials/03-poisson/systematic.html).
...
[^2]: Note here I'm using $x_j$, not $x_i$, and that $X\beta$ is shorthand for $\beta_0 + \beta_1 x_1 + \beta_2 x_2 + \beta_3 x_3$ and so on. In using the $j$ suffix, I'm referring to just one of the specific $x$ values, $x_1$, $x_2$, $x_3$, which is equivalent to selecting one of the *columns in* $X$. By contrast $i$ should be considered shorthand for selection of one of the *rows of* $X$, i.e. one of the series of observations that goes into the dataset $D$.

[^3]: $E(.)$ is the expectation operator, and $|$ indicates a condition. So, the two terms mean, respectively, *what is the expected value of the outcome if the variable of interest is 'switched on'?*, and *what is the expected value of the outcome if the variable of interest is 'switched off'?*
...
[^6]: The lower bound for the marginal effect of a discrete variable, or any variable, is zero. This is when the absolute value of the sum of the product of the other variables is infinite.
...
We can then ask the model to make predictions of $P(am=1)$ for a large number of values of `hp` and `wt` within the observed range, both in the condition in which `vs=0` and in the condition in which `vs=1`. The `expand_grid` function[^7] can help us do this:

[^7]: Or the base R `expand.grid` function
...
For an interactive companion to this material, the [GLM Dashboard](https://jonminton.github.io/glm-dashboard-explainer/) lets you work through the same framework hands-on — choosing distributions, link functions, and fitting models for different data types.
